...
def plot_training_metrics(csv_path):
    ...
    gs = fig.add_gridspec(2, 4, height_ratios=[1.3, 1], hspace=0.35, wspace=0.3)
    ...
    ax1.set_title('(a) Model Performance During GRPO Training', 
                  fontweight='bold', fontsize=13, pad=15)
    
    # Improved legend with clear explanation
    legend = ax1.legend(loc='lower right', frameon=True, fontsize=10,
    ...
    plot_with_smooth(ax2, df['step'], df['loss'], 
                     color='#1f77b4', label='Smoothed', window=smooth_window)
    ...
    plot_with_smooth(ax3, df['step'], df['kl'], 
                     color='#d62728', label='Smoothed', window=smooth_window)
    ax3.set_ylabel('KL Divergence', fontsize=11, fontweight='bold')
    ax3.set_xlabel('Training Steps', fontsize=10)
    ax3.set_title('(c) KL Divergence', fontsize=11, fontweight='bold')
    ...
    plot_with_smooth(ax4, df['step'], df['entropy'], 
                     color='#9467bd', label='Smoothed', window=smooth_window)
    ax4.set_ylabel('Policy Entropy', fontsize=11, fontweight='bold')
    ...
    plot_with_smooth(ax5, df['step'], df['grad_norm'], 
                     color='#ff7f0e', label='Smoothed', window=smooth_window)
    ax5.set_ylabel('Gradient Norm', fontsize=11, fontweight='bold')
    ...
    # Save figure
    output_path = csv_path.parent / 'training_metrics_visualization.png'
...
def main():
    csv_path = Path("D:/PythonEx/cs566_group_project/FinalReortDocs/sft+grpo-grpo-training_metrics.csv")
