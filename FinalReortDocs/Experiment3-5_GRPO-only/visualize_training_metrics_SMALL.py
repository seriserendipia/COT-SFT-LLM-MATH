"""
训练指标可视化脚本 - 实验3-5 (GRPO-only) - SMALL版本
字体配置：小号（适合论文单栏）
...
OUTPUT_PNG = "training_metrics_visualization_SMALL.svg"
...
def main():
    print("="*70)
    print("📊 实验3-5 (GRPO-only) 训练可视化 - SMALL版本")
    print("="*70)
    ...
    if 'loss' in df.columns:
        ...
        ax2.set_title('(b) Training Loss', fontsize=14, fontweight='bold')
        ax2.legend()
    ...
    if 'kl' in df.columns:
        ...
        ax3.set_title('(c) KL Divergence from Base Model', fontsize=14, fontweight='bold')
        ax3.legend()
    ...
    if 'entropy' in df.columns:
        ...
        ax4.set_title('(d) Policy Entropy', fontsize=14, fontweight='bold')
        ax4.legend()
    ...
    if 'grad_norm' in df.columns:
        ...
        ax5.set_title('(e) Gradient Norm', fontsize=14, fontweight='bold')
        ax5.legend()
    ...
    print(f"\n✅ 图表已保存到: {OUTPUT_PNG}")
    print(f"   字体配置: SMALL (主标题18pt, 子标题14pt, 轴标签13pt)")
